...
const USER_AGENT = "mosaic-test-data-image-validator/1.0 (+https://github.com/mosaic-code-coop/test-data-api)";
  ...
  describe(`${defaultOptions.datasetName} - Image URL Validation`, () => {
    ...
    async function fetchAndValidate(url: string): Promise<void> {
      ...
      if (/^image\/svg\+xml/.test(contentType)) {
        ...
        const bodyPreview = new TextDecoder("utf-8", { fatal: false }).decode(buffer.slice(0, 4096));
        ...
        if (!detected || !/^image\//.test(detected.mime)) {
          throw new Error(`body is not a recognised image (detected=${detected?.mime ?? "unknown"})`);
        }
      }
